...
def prepare_data(args):
    ...
    # 30000
    data_base_path = '/home/liuyuan/project/common_datasets/'
    # data_base_path = '/homec/liuyuan/datasets/fedbn_datasets/'
...
# for pfeddr
def train_multitask(model, data_loader, optimizer, loss_fun, device, alpha=0.5, beta=1):
    def multitaskLossFn(output_local, output_global, y, alpha):
        ...
        if (alpha > 0):
            ...
            loss = loss_fun(output_global, y) + loss_fun(output_local, y)
    ...
    for data, target in data_loader:
        ...
        loss = multitaskLossFn(output_local, output_global, target, alpha=alpha)
        KL_loss_g = torch.nn.functional.kl_div(torch.softmax(output_global, dim=1), torch.softmax(output_local, dim=1),
                                               reduction="batchmean")
...
# for pfeddr test
def test_ensemble_max(model, data_loader, loss_fun, device):
    model.eval()
    loss_all = 0
    total = 0
    correct = 0
...
def test_ensemble(model, data_loader, loss_fun, device):
    ...
    with torch.no_grad():
        for data, target in data_loader:
            ...
            output_bagging = (output_local + output_global) / 2
            pred = output_bagging.argmax(dim=1)

            loss = loss_fun(output_bagging, target)
            loss_all += loss.item()
            total += target.size(0)
            # pred = output.data.max(1)[1]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='../checkpoint/domainnet_pfeddr',
                        help='path to save the checkpoint')
    parser.add_argument('--resume', action='store_true', help='resume training from the save path checkpoint')

    parser.add_argument('--mode', type=str, default='pfeddr', help='name for dir')

    parser.add_argument('--seed', type=int, default=0, help='seed')

    parser.add_argument('--beta', type=float, default=0, help='The hyper parameter for pfeddr')
    ...
    #  python pfeddr_domainnet.py --log --mode pfeddr --unseen_client 3 --gpu 2
    ...
    exp_folder = exp_folder + "_beta{}".format(args.beta)
    ...
    log = args.log
    if log:
        log_path = os.path.join('../logs/domainnet_pfeddr', exp_folder)
    ...
    if args.test:
        ...
        # /home/liuyuan/liuyuan_home/project/FedBN/checkpoint/domainnet_pfeddr/pfeddr_beta2
        checkpoint = torch.load(
            '../checkpoint/domainnet_pfeddr/pfeddr_beta{}/{}_seed{}_unseen{}'.format(args.beta, args.mode.lower(), seed,
                                                                                    args.unseen_client))
        ...
        if args.mode.lower() == 'pfeddr':
            ...
            for client_idx, datasite in enumerate(datasets):
                if args.mode.lower() == 'pfeddr':
                    _, test_acc = test(pmodels[client_idx], test_loaders[client_idx], loss_fun, device, mode="local")
                else:
            ...
            for client_idx, datasite in enumerate(datasets):
                if args.mode.lower() == 'pfeddr':
                    _, test_acc = test(pmodels[client_idx], unseen_test_loader, loss_fun, device, mode="global")
                else:
    ...
    for a_iter in range(start_iter, args.iters):
        optimizers = [optim.SGD(params=pmodels[idx].parameters(), lr=args.lr) for idx
                      in range(client_num)]
        for wi in range(args.wk_iters):
            ...
            for client_idx, model in enumerate(pmodels):
                train_loss, train_acc = train_multitask(model, train_loaders[client_idx], optimizers[client_idx],
                                                        loss_fun, device, alpha=0.5, beta=args.beta)
...
#  python pfeddr_domainnet.py --log --mode pfeddr --unseen_client 5 --gpu 4
