...
def aggregate(args, server_model, models, client_weights):
    with torch.no_grad():
        # # aggregation for flop
        for key in server_model.state_dict().keys():
            # num_batches_tracked is a non trainable LongTensor and
            # num_batches_tracked are the same for all clients for the given datasets
            if 'num_batches_tracked' in key:
                server_model.state_dict()[key].data.copy_(models[0].state_dict()[key])
            elif 'bn3' in key or 'bn4' in key or 'fc' in key:
                # print(key)
                pass
            else:
        ...
        for client_idx in range(len(models)):
            for key in server_model.state_dict().keys():
                if 'bn3' in key or 'bn4' in key or 'fc' in key:
                    pass
                else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='../checkpoint/digits_dapfl', help='path to save the checkpoint')
    parser.add_argument('--resume', action='store_true', help='resume training from the save path checkpoint')
    parser.add_argument('--mode', type=str, default='fedper', help='fedper')
    ...
    SAVE_PATH = os.path.join(args.save_path,
                             '{}-seed{}-unseen{}'.format(args.mode, seed, args.unseen_client))

    # path for log
    log = args.log
    if log:
        log_path = os.path.join('../logs/digits_dapfl/', exp_folder)
        if not os.path.exists(log_path):
            os.makedirs(log_path)
        logfile = open(os.path.join(log_path, '{}-seed{}-unseen{}.log'.format(args.mode, seed,
    ...
    p_models = [copy.deepcopy(server_model).to(device) for idx in range(client_num)]
    ...
    if args.test:
        ...
        if args.mode.lower() == 'fedbn':
            for k in range(client_num):
                p_models[k].load_state_dict(checkpoint['model_{}'.format(k)])
            for test_idx, test_loader in enumerate(test_loaders):
                _, test_acc = test(p_models[test_idx], test_loader, loss_fun, device)
    ...
    if args.resume:
        ...
        if args.mode.lower() == 'fedbn':
            ...
            for k in range(client_num):
                p_models[k].load_state_dict(checkpoint['server_model'])
    ...
    for a_iter in range(resume_iter, args.iters):
        optimizers_g = [optim.SGD(params=p_models[idx].parameters(), lr=args.lr) for idx in range(client_num)]
        for wi in range(args.wk_iters):
            ...
            for k in range(client_num):
                p_model, train_loader, optimizer_g = p_models[k], train_loaders[k], optimizers_g[k]
                train(p_model, train_loader, optimizer_g, loss_fun, client_num, device)

        # aggregation
        server_model, p_models = aggregate(args, server_model, p_models, client_weights)

        # report after aggregation
        for k in range(client_num):
            train_loss, train_acc = test(p_models[k], train_loaders[k], loss_fun, device)
            print(
        ...
        if (a_iter + 1) % 10 == 0 or a_iter > (args.iters - 20):
            ...
            for test_idx, test_loader in enumerate(test_loaders):
                test_loss, test_acc = test(p_models[test_idx], test_loader, loss_fun, device)
                print(
            ...
            for test_idx in range(len(p_models)):
                test_loss, test_acc = test(p_models[test_idx], unseen_test_loader, loss_fun, device)
    ...
    torch.save({
        'model_0': p_models[0].state_dict(),
        'model_1': p_models[1].state_dict(),
        'model_2': p_models[2].state_dict(),
        'model_3': p_models[3].state_dict(),
        # 'model_4': g_models[4].state_dict(),
