...
# for dapfl
def train_multitask(model, train_loader, optimizer, loss_fun, client_num, device, alpha=0.5, mode="global"):
...
def test_ensemble(model, test_loader, loss_fun, device):
    ...
    for data, target in test_loader:
        ...
        output, x = model(data, mode='local')
        output2, x2 = model(data, mode='global')
        logits_a = output.data.max(1)[0]
        logits_b = output2.data.max(1)[0]
        pred = output.data.max(1)[1]
        #
        # print(logits_a)
        # print(logits_b)
        # print(pred)
        # print(output2.data.max(1)[1])

        for index in range(len(data)):
            if logits_b[index] > logits_a[index]:
                pred[index] = output2.data.max(1)[1][index]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mtl_alpha', type=float, default=0.5, help='The hyper parameter for fedprox')
    parser.add_argument('--save_path', type=str, default='../checkpoint/digits_dapfl',
    ...
    if args.mode == 'dapfl':
        ...
        exp_folder += '_ordered_momentum0.5_202355'
    ...
    if args.test:
        ...
        #/home/liuyuan/liuyuan_home/project/FedBN/checkpoint/digits_dapfl/dapfl_+ordered_momentum0.5/dapfl_seed0_unseen0
        checkpoint = torch.load(f'../checkpoint/digits_dapfl/dapfl_+ordered_momentum0.5/dapfl_seed0_unseen{args.unseen_client}')
        ...
        for wi in range(args.wk_iters):
            ...
            for client_idx in range(client_num):
                ...
                if args.mode.lower() == 'fedprox':
                    ...
                    train_multitask(pmodel, train_loader, optimizer, loss_fun, client_num, device, args.mtl_alpha)
